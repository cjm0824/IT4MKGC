...
class Verbalizer:
    ...
    def json_verbalize(self, head, relation, tail=None, inverse=False): #将邻域三元组转化为 verbalization
        ...
        if inverse:#已知tail，预测head
            neighborhood = self.json_get_neighborhood(tail, relation, head, limit)#查询tail的邻域信息
...
entity_img_path = "E:\\MMKG\\KGC\\fb15k_wn18_images\\wn18-images"
...
def save_images_to_folder(image_files, folder_base_path, tail, grid_size=None, image_size=(224, 224), padding_color=(255, 255, 255)):
    ...
    try:
        ...
        # 构造保存路径
        save_path = os.path.join(folder_base_path, f"n{tail}-image.jpg")
        # 保存拼接后的图像
...
def verbalize_dataset(input_df, output_collection, verbalizer, json_file_path=None):
    ...
    for i, doc in tqdm(input_df.iterrows(), total=len(input_df)):
        try:
            ...
            head_path = os.path.join("wn18-Images_update", f"n{head}-image.jpg")
            if os.path.exists(head_path):
                ...
                if head_img_files is not None and len(head_img_files)>0:
                    save_images_to_folder(head_img_files, "wn18-Images_update", head)
                    image_information = "Following are images associated with {} in the multimodal knowledge graph: \n<Image>\n".format(
                        verbalizer.entity2text[doc['head']])
                    # image_information = "Following are images associated with {} in the multimodal knowledge graph: \n".format(
                    #     verbalizer.entity2text[doc['head']])
            ...
            tail_path = os.path.join("wn18-Images_update", f"n{tail}-image.jpg")
            if os.path.exists(tail_path):
                ...
                if tail_img_files is not None and len(tail_img_files)>0:
                    save_images_to_folder(tail_img_files, "wn18-Images_update", tail)
...
parser.add_argument("--relation_vectors_path", help="path to the embeddings of verbalized relations", default="WN18-fasttext_relation_emb.npy")
parser.add_argument("--rel2ind_path", help="path to the mapping of textual relations to the index of corresponding vectors", default="../dataset/WN18/relation2ind.json")
parser.add_argument("--entity_mapping_path", help="path to the entity2text mapping", default="../dataset/WN18/entity2text.txt")
...
parser.add_argument("--relation_mapping_path", help="path to the relation2text mapping", default="../dataset/WN18/relaion2text.json")
parser.add_argument("--mongodb_port", help="port of the mongodb collection with the dataset", type=int, default=27017)
parser.add_argument("--input_db", help="name of the mongo database that stores wikidata5m dataset", default='wn18-image')
...
with open(args.entity_mapping_path, 'r', encoding='utf-8') as f:
    for line in f.readlines():
        ...
        _id, name = line[0], line[1].split(",")[0]
...
with open(args.entity_mapping_path, 'r', encoding='utf-8') as f:
    for line in f.readlines():
        line = line.strip().split('\t')
        _id, name = line[0], line[1].split(",", 1)[1].strip()
        entity_description[_id] = name
...
training_path = "../dataset/WN18/train.tsv"
validation_path = "../dataset/WN18/dev.tsv"
testing_path = "../dataset/WN18/test.tsv"
# figure_path = "../dataset/WN18/figure.tsv"

train_df = pd.read_csv(training_path, sep='\t', names = ['head', 'relation', 'tail'], encoding='utf-8', dtype={'head': str, 'relation': str, 'tail': str})
valid_df = pd.read_csv(validation_path, sep='\t', names = ['head', 'relation', 'tail'], encoding='utf-8', dtype={'head': str, 'relation': str, 'tail': str})
test_df = pd.read_csv(testing_path, sep='\t', names = ['head', 'relation', 'tail'], encoding='utf-8', dtype={'head': str, 'relation': str, 'tail': str})
...
entity_dict_path = '../dataset/WN18/entity2id.txt'
...
train_json_path = "wn18-verbalizer_update-neighyes/train_verbalized_update.json"
...
valid_json_path = "wn18-verbalizer_update-neighyes/valid_verbalized_update.json"
...
test_json_path = "wn18-verbalizer_update-neighyes/test_verbalized_update.json"
